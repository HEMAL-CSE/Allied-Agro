...
class BeefFatteningTreatmentDesc extends StatefulWidget {
  const BeefFatteningTreatmentDesc({super.key});

  @override
  State<BeefFatteningTreatmentDesc> createState() => _BeefFatteningTreatmentDescState();
}

class _BeefFatteningTreatmentDescState extends State<BeefFatteningTreatmentDesc> {
  String? shed_id;
    ...
    for(var i in doctors) {
      if (i['selected'] == true) {
        ...
        MultipartRequest request = MultipartRequest(
            'POST', Uri.parse('http://68.178.163.174:5000/cattles/treatment'));

        // (MultipartFile('file', imageFile!.readAsBytes().asStream(), imageFile!.lengthSync()));
  ...
  void getData() async {
    ...
    final url = Uri.parse('http://68.178.163.174:5000/doctors/beef?farm_id=${farm_id}');
  ...
  void deleteData(cattle_id, doctor_id) async {
    final url = Uri.parse('http://68.178.163.174:5000/cattles/treatment/delete?cattle_id=${cattle_id}&doctor_id=${doctor_id}');
              ...
              child: Container(
                ...
                child: Column(
                  children: [ 
                    ...
                    Text('Cattle ID: ${i[i.keys.toList()[0]][0]['cattle_id']}', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20),),
                                ...
                                builder: (BuildContext context) => AlertDialog(
                                  ...
                                  actions: <Widget>[
                                    ...
                                    TextButton(
                                      onPressed: ()
                                      {
                                        deleteData(i[i.keys.toList()[0]][0]['cattle_id'],i.keys.toList()[0]);
                                        Navigator.pop(context, 'OK');
                                      },
                                      child: const Text('OK'),
                                    ),
