...
import 'package:alliedagro/Doctor/BeefFattening/DoctorBeefTreatment.dart';
import 'package:http/http.dart';
import 'package:shared_preferences/shared_preferences.dart';

class DoctorBeef extends StatefulWidget {
  const DoctorBeef({super.key});

  @override
  State<DoctorBeef> createState() => _DoctorBeefState();
}

class _DoctorBeefState extends State<DoctorBeef> {
  List<dynamic> data = [];
  ...
  void getData() async {
    ...
    final url = Uri.parse('http://68.178.163.174:5008/doctors/beef/doctor?user_id=${user_id}');
        ...
        children: [
          for(var i in data)
            GestureDetector(
              onTap: () {
                // print(i.keys[0]);
                Navigator.push(context, MaterialPageRoute(builder: (context) => DoctorBeefTreatment(cattle_id: i.keys.toList()[0],)));
              },
              child: Card(
                elevation: 5,
                child: Container(
                  ...
                  child: Column(
                    children: [
                      Text('Cattle ID: ${i.keys.toList()[0]}', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20),),
