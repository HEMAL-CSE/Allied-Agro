...
import 'package:alliedagro/Doctor/Chicken/DoctorChickenTreatment.dart';
import 'package:http/http.dart';
import 'package:shared_preferences/shared_preferences.dart';

class DoctorChicken extends StatefulWidget {
  const DoctorChicken({super.key});

  @override
  State<DoctorChicken> createState() => _DoctorChickenState();
}

class _DoctorChickenState extends State<DoctorChicken> {
  List<dynamic> data = [];
  ...
  void getData() async {
    ...
    final url = Uri.parse('http://68.178.163.174:5008/doctors/chicken/doctor?user_id=${user_id}');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: 'Chicken',),
      body: ListView(
        children: [
          for(var i in data)
            GestureDetector(
              onTap: () {
                // print(i.keys[0]);
                Navigator.push(context, MaterialPageRoute(builder: (context) => DoctorChickenTreatment(chicken_id: i.keys.toList()[0],)));
              },
              child: Card(
                elevation: 5,
                child: Container(
