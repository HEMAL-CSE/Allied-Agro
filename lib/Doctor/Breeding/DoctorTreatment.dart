...
class DoctorTreatment extends StatefulWidget {
  final cow_id;
  const DoctorTreatment({super.key, this.cow_id});

  @override
  State<DoctorTreatment> createState() => _DoctorTreatmentState();
}

class _DoctorTreatmentState extends State<DoctorTreatment> {
...
print(user_id);
    ...
    final url = Uri.parse('http://68.178.163.174:5008/doctors/breeding/cow?user_id=${user_id}&cow_id=${widget.cow_id}');
    ...
    List result = resbody
      ...
      return val;
        ...
        .map((e) => {e.key: e.value})
        .toList();
    print(result[0][result[0].keys.toList()[0]][0]['treatment_desc']);
    // print(result);
  ...
  void handleSubmit() async {
    ...
    final url = Uri.parse('http://68.178.163.174:5008/doctors/breeding/treatment?doctor_id=${user_id}');
