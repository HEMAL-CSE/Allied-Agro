...
class DoctorBreeding extends StatefulWidget {
  const DoctorBreeding({super.key});

  @override
  State<DoctorBreeding> createState() => _DoctorBreedingState();
}

class _DoctorBreedingState extends State<DoctorBreeding> {
  ...
  void getData() async {
    ...
    final url = Uri.parse('http://68.178.163.174:5008/doctors/breeding/doctor?user_id=${user_id}');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: 'Breeding',),
      body: ListView(
        children: [
          for(var i in data)
            GestureDetector(
              onTap: () {
                // print(i.keys[0]);
                Navigator.push(context, MaterialPageRoute(builder: (context) => DoctorTreatment(cow_id: i.keys.toList()[0],)));
              },
              child: Card(
                elevation: 5,
                child: Container(
