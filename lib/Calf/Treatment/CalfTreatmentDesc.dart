...
import 'package:alliedagro/components/VideoPlay.dart';
...
class CalfTreatmentDesc extends StatefulWidget {
  const CalfTreatmentDesc({super.key});

  @override
  State<CalfTreatmentDesc> createState() => _CalfTreatmentDescState();
}

class _CalfTreatmentDescState extends State<CalfTreatmentDesc> {
  String? shed_id;
  ...
  Future pickImages() async {
    ...
    print("Image List Length:" + fileList!.length.toString());
    setState(() {
      imageFiles = fileList;
    });
  }
  ...
  void handleSubmit() async {
    ...
    for(var i in doctors){
      ...
      if(i['selected'] == true){
        MultipartRequest request = MultipartRequest('POST', Uri.parse('http://68.178.163.174:5008/calf/treatment'));
  ...
  void getData() async {
    ...
    final url = Uri.parse('http://68.178.163.174:5008/doctors/calf?farm_id=${farm_id}');
  ...
  void deleteData(calf_id, doctor_id) async {
    final url = Uri.parse('http://68.178.163.174:5008/calf/treatment/delete?calf_id=${calf_id}&doctor_id=${doctor_id}');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: 'চিকিৎসা',),
      body: ListView(
        children: [
          SizedBox(height: 05,),
          ...
          SizedBox(height: 05,),
          CustomTextField(controller: diseases_description, hintText: 'রোগের বিস্তারিত', obscureText: false, textinputtypephone: false, maxLines: 8,),
          ...
          if(imageFiles != null)
            GridView.builder(
                ...
                itemBuilder: (BuildContext context, int index) {
                  return isVideo(imageFiles![index].path) ? VideoPlay(path2: imageFiles![index],)  : Image.file(File(imageFiles![index].path),
                    fit: BoxFit.cover,);
                }),
          Row(
            ...
            children: [
              ...
              Container(
                ...
                child: ElevatedButton(
                    ...
                    }, child: const Text("ডাক্তারকে পাঠান")),
              ),
              Spacer(),
            ],
          ),
          ...
          for(var i in data)
            Card(
              ...
              child: Container(
                ...
                child: Column(
                  children: [
                    Text('গাভী নাম্বার: ${i[i.keys.toList()[0]][0]['calf_id']}', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20),),
                    Text('ডাক্তার নাম্বার: ${i.keys.toList()[0]}', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20),),
                    ...
                    GridView.builder(
                        ...
                        gridDelegate:
                        SliverGridDelegateWithFixedCrossAxisCount(
                            crossAxisCount: i[i.keys.toList()[0]]!.length > 2 ? 2 : i[i.keys.toList()[0]]!.length
                        ),
                        itemBuilder: (BuildContext context, int index) {
                          if(isVideo(i[i.keys.toList()[0]][index]['image_url']) == false){
                                            ...
                                            child: ListView(
                                              ...
                                              children: [
                                                for(var j in i[i.keys.toList()[0]])
                                                  isVideo(j['image_url']) == false ?
                                                  Column(
                                                    children: [
                                                      Container(height: 200, width: 200,child: Image.network(j['image_url'], fit: BoxFit.fitWidth)),
                                                      SizedBox(height: 10,)
                                                    ],
                                                  )
                                                      : VideoPlay(pathh: j['image_url'],),
                                ...
                                builder: (BuildContext context) => AlertDialog(
                                  ...
                                  actions: <Widget>[
                                    ...
                                    TextButton(
                                      onPressed: ()
                                      {
                                        deleteData(i[i.keys.toList()[0]][0]['calf_id'],i.keys.toList()[0]);
                                        Navigator.pop(context, 'OK');
                                      },
                                      child: const Text('OK'),
                                    ),
